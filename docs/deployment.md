...
首次部署、换服务器、重装 `bdpan` 或夸克 skill 后，都需要在管理端完成一次授权。打开“网盘账号”，分别新增百度和夸克账号，点击“授权”获取链接，授权完成后把页面返回的授权码粘贴回后台。每个网盘类型都支持多账号，并各自设置一个默认账号；上传处理默认使用该类型的默认账号同步网盘，上传页和资源库处理弹窗也可以为当前批次临时指定百度/夸克账号。
